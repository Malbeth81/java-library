...
public class Noise {
    ...
    public static double[] cut(double[] noise, double cut) {
        if (noise != null && noise.length > 0) {
            double minValue = noise[0];
            double maxValue = noise[0];
            for (double aNoise : noise) {
                if (aNoise < minValue)
                    minValue = aNoise;
                else if (aNoise > maxValue)
                    maxValue = aNoise;
            }

            return cut(noise, cut, minValue, maxValue);
        }

        return null;
    }

    public static double[] cut(double[] noise, double cut, double min, double max) {
        if (noise != null && noise.length > 0) {
            double[] result = new double[noise.length];

            for (int i = 0; i < noise.length; i++)
                result[i] = (noise[i] < (max - min) / (1 / cut) ? 1 : 0);
    ...
    public static double[] perlinNoise(int width, int height, double scale) {
        ...
        for (int i = 0; i < width; i++) {
            for (int j = 0; j < height; j++) {
                double x = i * scale / width;                                // FIND RELATIVE X,Y,Z
                double y = j * scale / height;                                // OF POINT IN CUBE.
